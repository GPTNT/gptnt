...
def known_configs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(params, "discover_players", lambda: ["gpt-4o", "claude-sonnet", "dummy"])
    monkeypatch.setattr(params, "discover_providers", lambda: ["openai", "anthropic"])

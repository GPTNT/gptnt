...
def _turn(role: str, observations: int, index: int) -> list[ModelMessage]:
    content: list[TextPart | BinaryContent] = [
...
def test_render_is_pure_and_windows_observations(
    ...
    for index in range(turns):
        ...
        assert image_count([requests[index + manual_offset]]) == int(
...
def test_eviction_preserves_manual_images_and_rendered_history() -> None:
    ...
    assert image_count(conversation.entries[0].messages) > 0
    assert image_count(conversation.entries[1].messages) == 0
    assert image_count(conversation.entries[2].messages) == 3
...
def test_evict_binary_content_keeps_last_per_part() -> None:
    ...
    assert image_count(remove_binary_content_from_messages(messages, keep_last=True)) == 1
    assert image_count(remove_binary_content_from_messages(messages, keep_last=False)) == 0
...
def test_window_keeps_last_image_only_in_recent_non_pinned_entries() -> None:
    ...
    assert [image_count(entry.messages) for entry in windowed] == [0, 0, 1]
    assert [image_count(entry.messages) for entry in entries] == [2, 2, 2]

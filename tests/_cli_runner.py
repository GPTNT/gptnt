...
turns the `SystemExit` cyclopts raises on completion, help, or parse errors into an exit code.
...
def invoke_cli(app: App, argv: list[str]) -> CliResult:
    ...
    consoles do not emit ANSI escapes, and a test can assert on the rendered text directly.
    """
    buffer = io.StringIO()
    exit_code = 0

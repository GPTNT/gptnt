...
In async mode the experiment wall-clock budget is shared between model inference and the
benchmark harness (observation fetch/render, Set-of-Marks, input building, action dispatch).
Every phase is already wrapped in a named logfire span, but the player/game processes run
`logfire.configure(..., send_to_logfire=False)` with no exporter attached, so those span
durations are discarded.

This module provides a :class:`SpanTimingExporter` that appends the durations of an allowlisted
set of spans to a per-process JSONL file. Attaching it as an `additional_span_processor` is
behaviour-neutral and gated behind `ObservabilitySettings.capture_span_timings` (off by
default), so normal runs are unchanged and the measurement does not perturb the wall-clock it
measures.

The per-step accounting has two levels of overhead::

    pydantic_ai_s  = sum(duration("chat *") for pydantic-ai-scoped spans)
    agent_s        = duration("Send request to agent")
    agent_overhead = agent_s - pydantic_ai_s   # pydantic-ai framework + tool orchestration
    player_overhead = duration("player forward pass") - agent_s

See `gptnt timing` (`gptnt.records.cli.timing`) for the query side.
"""
...
class _TimingIdentity:
    ...
    Each player/game process handles essentially one experiment for its whole lifetime, so a
    single module-level instance is sufficient and avoids threading identity through every span.
    Populated once from the service's "configure for experiment" handler via
    :func:`set_timing_identity`; any field left unset is written as ``null``.
    """
...
def set_timing_identity(**updates: str | None) -> None:
    ...
    `player_name`, `model_name`, `game_uuid`) as keyword arguments. Unknown keys are ignored
    so callers can pass through whatever context they have without coupling to this schema.
    """
...
def _is_captured(span: "ReadableSpan") -> bool:
    ...
    Captures both the explicit allowlist (player/game spans) and pydantic-ai
    "chat *" spans, which represent the actual LLM call inside `agent.run()`.
...
class SpanTimingExporter(SpanExporter):
    ...
    player/game processes never contend on a shared writer — this mirrors the experiment
    recorder's per-process pattern.

...
class StepContextRecorder(Protocol):
    ...
    Declared in the player layer so that input building does not depend on the records
    package. The concrete recorder (`ExperimentPlayerRecorder` in `gptnt.records`)

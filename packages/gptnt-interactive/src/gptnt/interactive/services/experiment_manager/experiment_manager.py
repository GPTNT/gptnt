...
from gptnt.interactive.services.registry.manifest import (
    GameServiceManifest,
    PlayerServiceManifest,
    ServiceManifest,
    ServiceState,
)
from gptnt.interactive.services.registry.registry import ObservableServiceRegistry
...
class ExperimentManager(ObservableServiceRegistry):
    ...
    async def lifespan(self) -> AsyncGenerator[None]:
        ...
        async with self.redis_broker, super().lifespan(), anyio.create_task_group() as tg:
            ...
            tg.start_soon(self.matchmaking_loop)
    ...
    async def matchmaking_loop(
        self, *, interval: float = 1, max_iterations: int | None = None
    ) -> None:
        """Continuously attempt to match experiments."""
        iteration = 0
        async for _ in periodic(interval):
            await self.try_match_experiments()
            await self.cleanup_finished_sessions()
            iteration += 1
            if max_iterations is not None and iteration >= max_iterations:
                break

    async def try_match_experiments(self) -> None:

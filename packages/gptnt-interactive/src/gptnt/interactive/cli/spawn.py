...
from gptnt.interactive.cli.orchestrator import (
    ProcessOrchestrator,
    abort_on_shutdown,
    fail_and_terminate,
)
...
async def _wait_for_em_ready(
    ...
    console.print("  Waiting for experiment manager to be ready...")
        ...
        with anyio.fail_after(fail_after):
            async for _ in periodic(interval):
                ...
                if response.is_success:
                    console.print("  Experiment manager is ready.")
                    return

    console.print(
        f"[bold red]Experiment manager did not become ready within {fail_after:.0f}s.[/bold red]"
    )

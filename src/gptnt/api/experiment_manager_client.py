...
class ExperimentManagerClient(BaseClient):
    ...
    @asynccontextmanager
    async def connect(
        self, *, connection: PlayerAPIInfo | RoomManagerAPIInfo
    ) -> AsyncGenerator[None]:
        ...
        switcher = {PlayerAPIInfo: self.connect_player, RoomManagerAPIInfo: self.connect_room}
    ...
    async def connect_room(self, connection: RoomManagerAPIInfo) -> bool:
        """Connect to the ExperimentManager.
    ...
    async def connect_player(self, connection: PlayerAPIInfo) -> bool:

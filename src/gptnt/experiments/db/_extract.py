...
from gptnt.experiments.models import ExperimentMetadata, ExperimentStepRecord, is_valid_experiment
...
def grab_last_bomb_state_from_experiment_file(file_path: Path) -> BombState | None:  # noqa: WPS212
    ...
    - `mm.rfind()` fails = shouldn't raise, but a truncated/corrupted file could raise ValueError
...
def scan_is_hard_crash(paths: list[Path]) -> bool:
    """True if any of the grouped player files recorded a hard crash."""
    for path in paths:
        with path.open("rb") as open_file:
            if next(ijson.items(open_file, "is_hard_crash"), False):
                return True
    return False


def scan_final_bomb_state(paths: list[Path]) -> BombState | None:
    """The last recorded bomb state across the grouped player files, or None if there is none."""
    for path in paths:
...
def compute_experiment_validity(paths: list[Path]) -> bool:
    ...
    experiment that never reached a bomb state (e.g. crashed before the first step) is not valid.
    This is the same notion of validity the DB ingestion stamps, so disk-only callers (the local
    ledger, local cleanup) and the DB agree without consulting W&B.
    """
    final_bomb_state = scan_final_bomb_state(paths)
    if final_bomb_state is None:
        return False
    return is_valid_experiment(
        is_hard_crash=scan_is_hard_crash(paths), final_bomb_state=final_bomb_state
    )
...
def extract_metadata_from_paths(paths: list[Path]) -> DumpedExperimentMetadata:
    ...
    is_hard_crash = scan_is_hard_crash(paths)
    final_bomb_state = scan_final_bomb_state(paths)
...
def filter_existing_experiments(
    ...
    *,
    ...
    Compares against `file_names` in `experiment_metadata` so that ingestion is idempotent and safe

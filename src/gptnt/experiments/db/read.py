...
type _UsageCells = list[bytes | bytearray]
...
def load_experiment_summaries(
    ...
    *,
    ...
    if names:
        clauses.append(f"defuser_name IN ({', '.join('?' for _ in names)})")
        params.extend(names)
...
def _group_cells_by_session(
    rows: list[tuple[UUID, PlayerRole, str | None, bytes | bytearray]],
    ...
    Bomb states are per session (only the defuser's steps have one); usage cells are further split

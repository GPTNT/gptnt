...
def _create_and_checkout_branch(
    ...
    # Force so the working tree and index reset to `base`, dropping the previous bundle's files.
...
def _push_and_open_pr(
    *,
    ...
    """Force-push one bundle's branch and open (or find) its PR, returning the PR URL."""
...
class SubmissionResult:
    bundle: str
    """The top-level bundle directory this pull request covers."""
    ...
    error: str | None = None
    """Set when this bundle failed to push or open its PR."""
...
def _submit_one_bundle(session: _SubmissionSession, bundle_dir: Path) -> SubmissionResult:
    ...
    try:
        ...
        return SubmissionResult(
            bundle=bundle_dir.name, branch=branch_name, pr_url="", error=str(exc)
        )
    return SubmissionResult(bundle=bundle_dir.name, branch=branch_name, pr_url=pr_url)

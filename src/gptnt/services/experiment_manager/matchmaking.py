...
from gptnt.services.registry.manifest import PlayerServiceManifest
...
def find_valid_pairing_for_single_player(
    *, available_players: list[PlayerServiceManifest], experiment: ExperimentSpec
    ...
    for player in available_players:
        if player.heartbeat.capabilities.player_name == experiment.defuser_name:
...
def find_valid_pairing_for_multi_player(
    *, available_players: list[PlayerServiceManifest], experiment: ExperimentSpec
    ...
    for player1, player2 in itertools.permutations(available_players, 2):
        if (
            player1.heartbeat.capabilities.player_name == experiment.defuser_name
            and player2.heartbeat.capabilities.player_name == experiment.expert_name
        ):
...
def get_playable_pairings(
    *, available_players: list[PlayerServiceManifest], available_experiments: list[ExperimentSpec]

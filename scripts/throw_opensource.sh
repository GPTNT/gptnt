...
# usage: ./throw_opensource.sh <number_of_rooms> <claude37_bedrock_players> <gemini_25_players> <gpt4o_players> <internvl3_players> <qwen25_players>
...
if [[ -z $NUM_ROOMS || $NUM_ROOMS -lt 1 || -z $INTERNVL3_PLAYERS || $INTERNVL3_PLAYERS -lt 0 || -z $QWEN25_PLAYERS || $QWEN25_PLAYERS -lt 0 ]]; then
	echo "Usage: $0 <number_of_rooms> <internvl3_players> <qwen25_players>"
	exit 1
fi
...
# spawn internvl3 players
for ((i = 0; i < INTERNVL3_PLAYERS; i++)); do
	echo "THROWING: Starting internvl3 player $i..."
	run_and_track "internvl3_player_$i" env WANDB_RUN_GROUP=THROWING uv run python src/gptnt/entrypoints/run_player.py model=internvl3
	sleep 1
done

# spawn qwen25 players
for ((i = 0; i < QWEN25_PLAYERS; i++)); do
	echo "THROWING: Starting qwen25 player $i..."
	run_and_track "qwen25_player_$i" env WANDB_RUN_GROUP=THROWING uv run python src/gptnt/entrypoints/run_player.py model=qwen25
	sleep 1
done

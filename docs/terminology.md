...
`gptnt build-db` later derives the query surface from those files: one `ExperimentSummary` row
per attempt and the `experiment_step` table of every step. The parquet is always written; the DuckDB
tables are rebuilt from it. See [Tracking experiments](concepts/tracking-experiments.md) for the write and ingest contract.
...
- **`ledger`** — what answers "which experiments are already done?". The local ledger reads this
  ...
  through `is_valid_outcome`. See [Tracking experiments](concepts/tracking-experiments.md).

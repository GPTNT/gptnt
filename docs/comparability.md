...
- **Suite** (`configs/suites/<name>.yaml`) — what is measured. A frozen mission set, the per-role
...
configs/missions/recipes/        *.json files      PlayerCapabilities
                                      ▲               └── fingerprint ──────────┐
suite   configs/suites/<name>.yaml    │                                         │
  missions_path ──────────────────────┘                                        │
  defuser_protocol, expert_protocol   (PlayerProtocol)                          │
  matchup, modality, name, revision                                            │
                                                                               │
...
| ---------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------- |
| suite                              | the frozen definition of what is measured                      | `configs/suites/<name>.yaml` (`Suite`)                     |
| name                               | the suite's identity; equals its filename                      | `Suite` field                                              |
| revision                           | a monotonic counter; bumped on any change to what it measures  | `Suite` field                                              |
...
| comparability key    | `suite_name` + `suite_revision` + `capability_fingerprint` | on `ExperimentSummary`   |
...
Two recorded results are comparable when all three match:
    ...
    suite_name  +  suite_revision  +  capability_fingerprint

Reproducing a result also needs `gptnt_version` and `git_sha`.

The three meet only on the recorded result (`ExperimentSummary`). `suite_name` and `suite_revision`
...
The same test checks that a suite's `name` equals its filename, so a `run.yaml` reference and the
recorded `suite_name` stay in sync.
